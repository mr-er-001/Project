...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['invoice_no'])) {
    ...
    while ($item = mysqli_fetch_assoc($itemsRes)) {
        ...
        // Update stock (Return adds stock back)
    ...
    // Insert client transaction for return (credit)
    mysqli_query($conn, "
        INSERT INTO client_transactions 
            (invoice_no, client_id, total_amount, debit_amount, credit_amount, cash_mode, tdate)
        VALUES 
            ('{$next_return}', '{$client_id}', 
            (SELECT total_amount FROM client WHERE id='{$client_id}'), 
            0, '{$grand_total}', 1, NOW())
    ");

    echo "<script>alert('✅ Invoice returned successfully'); window.location.href='client_report.php';</script>";
    exit;
}
?>
